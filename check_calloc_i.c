...
void help(void){
	printf("USAGE:\n");
	printf(" check_calloc_i [-h] [-s] [-c] size=<size> itv=<interval>.\n");
	printf("  -h : help.\n");
	printf("  -s : stat.\n");
	printf("  -c : check args.\n");
	printf("  size : calloc size (4Byte:int).\n");
...
struct options *alloc_options(void){
	struct options *p;
	if((p = calloc(1,sizeof(struct options))) == NULL){
		printf("failed : calloc() in alloc_options().\n");
		exit(1);
	}
	return(p);
}
...
int main(int argc, char **argv){
	...
	/* test */
	printf("alloc\n");
	if( (ptr = calloc( (*opt).size, (size_t)sizeof(int) )) == NULL ){
