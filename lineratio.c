...
void print_help(){
	printf("USAGE:\n");
	printf("  lineratio if=<input file> [rskip=<skip size>] [cskip=<skip size>] [sum=<sum>] [c=<column size>] [ll=<line length>] [tl=<term length>] [-c] [-h]\n");
	printf("\n");
}
...
int main(int argc, char **argv){
	...
	while((c = fgetc(IFP)) != EOF){
		if(c == LF){
			...
			printf("%.10f",vec[0] * ((opt.sum)/sum));
			for(x=1;x<k;x++){
				printf(" %.10f",vec[x] * ((opt.sum)/sum));
			}
