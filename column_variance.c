...
int main(int argc, char **argv){
	...
	if((strcmp(argv[0],"column_varianceMLE") == 0)||(strcmp(argv[0],"./column_varianceMLE") == 0)){
		for(i=0;i<columns;i++){
			printf("%f ",(float)v_columns[i]/lines);
		}
	}else if((strcmp(argv[0],"column_variance") == 0)||(strcmp(argv[0],"./column_variance") == 0)){
		for(i=0;i<columns;i++){
			printf("%f ",(float)v_columns[i]/(lines - 1));
		}
	}
	printf("\n");
	return(0);
