...
int main(int argc, char **argv){
	...
	while(in_stream >= 1){
		...
		if(C >= 0x00 && C <= 0x7f){ /* ASCII */
			...
			utf_buf[i] = '\0';
			//write to O
			O = ((int)utf_buf[0] & 0x1f);
			O = O << 6;
			tmpO = (int)utf_buf[1] & 0x3f;
			O = (O | tmpO);
			fprintf(stdout,"&#x%05x;",O);
			is_put = 0;
